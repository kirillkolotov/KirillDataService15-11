...
namespace ViewModel
{
    public class UserDB : BaseDB
    {
        ...
        {
            command.CommandText = "INSERT INTO TblUsers (firstname,lastname,email,username,[password],IsAdmin) VALUES (@firstname,@lastname,@email,@username,@password,@IsAdmin)";
            LoadParameters(user);
            return ExecuteCRUD();
        }
        public int Update(User user)
        {
            command.CommandText = "UPDATE TblUsers SET firstname = @firstname,lastname = @lastname,email = @email,username = @username,[password] = @password,IsAdmin=@IsAdmin WHERE id = @id";
            LoadParameters(user);
            return ExecuteCRUD();
        }
